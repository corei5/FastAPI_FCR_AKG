...
def corpus_embeddings_ROBERTA_large(parameter):
...
def corpus_embeddings_BERT(parameter):
    claim_file_path = 'Dataset/cord19_ClaimKG_With_Conceptnet_Entities_For_DbpediaSpotlight.csv'
...
def transformers_claim_for_ROBERTA_large(top_k: int, search_terms):
    try:
...
def transformers_claim_for_BERT(top_k: int, search_terms):
    try:
...
def TfidfVectorizer_with_DBpedia_spotlight(top_k: int, search_terms):
    min_df = 32
    claim_file_path = 'Dataset/cord19_ClaimKG_With_Conceptnet_Entities_For_DbpediaSpotlight.csv'
    claim_df = pd.read_csv(claim_file_path)
    try:
...
def CountVectorizer_with_DBpedia_spotlight(top_k: int,search_terms):
    min_df = 32
    claim_file_path = 'Dataset/cord19_ClaimKG_With_Conceptnet_Entities_For_DbpediaSpotlight.csv'
    claim_df = pd.read_csv(claim_file_path)
    try:
